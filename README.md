...
> 아래 다이어그램은 라이트/다크 테마를 자동으로 따라갑니다.
> 인터랙티브 버전(테마 토글 · 4× PNG/SVG 내보내기): [docs/](docs/) 폴더의 같은 이름 `.html`

### 시스템 구성 — 무엇을 만들었나

![JB Compliance 아키텍처](docs/jb-compliance-architecture.svg)

담당자 → Next.js 워크스페이스(KM/EN/KO) → FastAPI 심의 파이프라인(맥락 추출 → 조항 매칭 → LLM Judge) → Neo4j 정책 지식그래프. 심의 경로는 그래프에 대해 항상 **읽기 전용**이다.

### 심의 1건의 여정 — 어떻게 동작하나

![광고 심의 요청 시퀀스](docs/jb-review-sequence.svg)

접수 → 맥락 추출 → 조항 매칭(템플릿 쿼리 우선, Text2Cypher 폴백) → 위반 판정 → 3언어 번역. 한 요청에 LLM 3회 호출, 판정·수정문은 근거 조항 ID를 인용한다.

### 정책 지식그래프 파이프라인 — 도메인 지식을 어떻게 구조화했나

![정책 KG 데이터플로](docs/jb-kg-dataflow.svg)

법령·감독규정·협회 가이드라인·상품 정보를 배치 컴파일해 80,921 노드 · 116 메타관계 · 17 분류체계의 그래프로 적재하고, 심의(매칭·Judge)와 온톨로지 탐색기가 읽기 전용으로 소비한다.
...
```bash
cp .env.example .env
# 심사위원(채점자)용: .env.example 에 데모 Neo4j(Aura) 접속정보가 이미 채워져 있어
# 복사만 하면 정책·상품·컨텍스트 그래프에 바로 연결된다(그래프 적재 불필요).
# 아래 LLM 키 하나만 본인 것으로 채우면 즉시 실행 가능:
#   OPENAI_API_KEY=...          (기본 판정/추출/임베딩)
#   ANTHROPIC_API_KEY=...       (Claude 판정 모델을 쓸 경우 — 선택)
```

`.env.example`에 모든 설정 항목과 기본값이 주석과 함께 정리되어 있다
(데모 Neo4j 접속정보, 로컬 LLM 라우팅, 캄보디아 워크스페이스 2-DB,
이미지 생성 품질/타임아웃, Track C 토글 등).

> **심사위원 Quick Path:** `cp .env.example .env` → `OPENAI_API_KEY`만 채움 →
> 백엔드(:8770) + 프론트(:3000) 기동 → 콘솔에서 광고 문안 붙여넣고 심사.
> 데모 Aura 인스턴스에 정책/상품 그래프가 이미 적재돼 있어 "정책 그래프 적재"
> 단계(아래)는 자체 DB로 재현할 때만 필요하다.
