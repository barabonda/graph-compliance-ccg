...
## 변경 일자: 2026-07-04 · 대상 기능: 상품 문서 구조화 그래프 적재 (CC-13)
- **변경 내용**: 신규 `structure_ppcbank_docs.py` — 상품 문서를 `full_text` 블롭이 아니라 **구조화 그래프(하이브리드 GraphRAG)**로 분해. 문서를 타입 섹션(overview/promotion/rate_table/conditions/requirements/fees/…)으로 나누고 표를 셀 단위 노드로: `(:ProductDocument)-[:HAS_SECTION]->(:DocSection)-[:HAS_RATE]->(:RateEntry {term_months,currency,channel,payment,rate_kind,holding_period,amount_tier,rate_pa})`, `-[:HAS_ELIGIBILITY]->(:EligibilityItem)`, `-[:HAS_CONDITION]->(:LoanCondition)`, `-[:HAS_FEE]->(:FeeItem)`, 산문은 `-[:HAS_CHUNK]->(:DocChunk)-[:NEXT]->`. Claude Opus 4.8로 21문서 추출 → KH workspace MERGE(idempotent·격리). 규모: DocSection 112·DocChunk 262(NEXT)·**RateEntry 202**·EligibilityItem 99·LoanCondition 64·FeeItem 51. CLI `--structure/--ingest/--verify/--all` + `--replace`. 라이브 심사·기존 ProductFact 그래프와 공존(무영향).
- **변경 사유**: 원격 팀원이 Aura에서 text2cypher/쿼리로 문서를 조회하려면 표가 텍스트에 묻히지 않고 **그래프-쿼리로 직접** 나와야 함(예: 12mo USD 디지털 만기 정기예금 금리 → `RateEntry{term_months:12,currency:'USD',channel:'digital',payment:'at_maturity'}` = 4.40). 검증: 정기예금 금리 매트릭스 원문 셀 단위 대조 **12/12**, 24-에이전트 적대적 감사 **GOOD 85/100·금리셀 178/178 정확·BLOCKER 0**. 감사 지적 2건 즉시 수정 — RateEntry에 `amount_tier`(잔액구간)·`rate_kind`/`holding_period`(중도해지) 차원 추가 → **Songkhoem 중도해지 40셀 복원**(12+40=52), VIP 잔액구간 보존, Heng Heng 8티어를 RateEntry로 이동(금리성 conditions 0). 재검증: RateEntry 202(중도해지 40·잔액티어 28), 정기예금 회귀 4.40 유지.
...
- **자체완결화(2026-07-04)**: 원격 팀원이 Aura에 직접 접속해 text2cypher/쿼리로 문서를 조회할 때 로컬 `.md/.txt` 파일 없이는 본문을 못 보는 문제 → `build_ppcbank_product_graph.py`가 각 `ProductDocument`에 **크롤 전문을 `full_text`(+`content_chars`) 속성으로 임베드**하도록 빌더 파이프라인에 추가(파일에서 nav/footer·PoC 헤더 제거 후 본문 verbatim). 재적재로 21개 문서 전부 채움(평균 1,712자). 원격 검증: 로컬 파일 없이 `MATCH (p:Product{name})-[:HAS_PRODUCT_DOCUMENT]->(d) RETURN d.full_text`로 문서 전문 반환 확인. 기존 KR 설계(포인터만)보다 개선 — 원격 조회 시나리오에 맞춤.
