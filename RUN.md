...
## 사전 요구사항
- **Python >= 3.10**  ⚠️ 현재 머신은 **3.9.6** → 백엔드 실행 전 업그레이드 필요
  (`requirements.txt`의 fastapi 0.135 / pydantic 2.12 등이 3.10+ 요구)
...
```bash
cd <repo>                       # = .../junbub/repo
python3 -m venv ../.venv        # ★ Python 3.10+ 인터프리터로 생성 (venv는 repo 바깥 junbub/.venv)
...
- **백엔드: ⏸ 미검증** — 시스템 Python 3.9.6 < 3.10 으로 의존성 설치 불가.
  Python 3.10+ 설치 후 `pip install -r requirements.txt` → `uvicorn server:app --port 8770` 로
  `/health` 부팅 확인 가능. (전체 심사 동작은 OpenAI 키 + Neo4j 필요.)
