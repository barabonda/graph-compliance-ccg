...
function StatMini({ label, value, sub, danger }: { label: string; value: string | number; sub?: string; danger: boolean }) {
  return (
    <div className="text-center">
      <div className="text-[10.5px] font-semibold whitespace-nowrap text-ink-4">{label}</div>
      <div className="mt-1">
        <span className={`text-[21px] font-extrabold ${danger ? "text-reject" : "text-pass"}`}>{value}</span>
...
export function ExecSummary({ result, resolved }: { result: ReviewOutput; resolved: Set<string> }) {
  ...
  return (
    <div className="flex shrink-0 items-stretch gap-4 rounded-[14px] border border-line bg-surface px-4.5 py-3.5 shadow-card">
      <div className="flex flex-col justify-center gap-2 border-r border-line pr-4.5">
        <div className="text-[10.5px] font-bold tracking-wider text-ink-4">AI 판정 · 권고</div>
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center">
        <div className="mb-1 text-[11px] font-bold tracking-wider text-ink-4">요약 사유</div>
        <p className="m-0 line-clamp-3 text-[13px] leading-relaxed text-ink-2">{result.rationale}</p>
      </div>
      <div className="flex items-center gap-4 border-l border-line pl-4.5">
        <StatMini label="미해소 이슈" value={open} sub={` / ${cards.length}`} danger={open > 0} />
        <StatMini label="위반 의심" value={highOpen} danger={highOpen > 0} />
        <StatMini label="필수 고지" value={`${met}/${checks.length}`} danger={checks.length > 0 && met < checks.length} />
      </div>
    </div>
  );
}
